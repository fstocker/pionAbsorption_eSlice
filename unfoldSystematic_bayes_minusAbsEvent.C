...
//
//Variation of Absorption Geant XS
// Add / remove 10% of Absorption Events --> Affects response matrix
//
// Add --> Remove Other Pion Inel Such that more Abs is there
//==============================================================================

void unfoldSystematic_bayes_minusAbsEvent(const string mcFilepath, const string dataPath, bool doMC = false, bool doXS = true)
{
   ...
   //===================================================================================== 
   // Decrease Absorption events by 10%          
   //=====================================================================================  
   //
   int total_abs = *frame_train.Filter("truePion_abs_response || truePion_abs_miss").Count();
   std::cout << "There are total = " << total_abs << " Absorption response and miss events" << std::endl;
   //int ninety_percent_abs = 0.9*total_abs;
   //std::cout << "90 percent are = " << ninety_percent_abs << std::endl;
   int ninety_percent_abs = 0.8*total_abs;
   std::cout << "80 percent are = " << ninety_percent_abs << std::endl;
   //counter in how many filled response and miss function. decrease until it is 0 then miss and response cannot be filled anymore

   frame_train
      .Foreach( [ &response_abs, &ninety_percent_abs ](double true_initE, double true_interE, 
               double reco_initE, double reco_interE, 
               bool ev_response, bool ev_miss, bool ev_fake){

            //Signal
            if(ev_response && ninety_percent_abs != 0){
            response_abs.Fill(reco_interE, true_interE);
            --ninety_percent_abs;
            }
            //MisReconstruction
            else if( ev_miss && ninety_percent_abs != 0) {
   ...
   TH1D* hMeas_data_initE= new TH1D ("syst_minusAbsEvent_initKE", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_interE= new TH1D ("trueMC_interE", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_interE= new TH1D ("syst_minusAbsEvent_interKE", "Measured data Run 58XX interE; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_abs= new TH1D ("trueMC_abs", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_abs= new TH1D ("syst_minusAbsEvent_abs", "Measured data Run 58XX absorption; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   hUnfold_initE->SetNameTitle("hSystUnfold_minusAbsEvent20_initE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_interE->SetNameTitle("hSystUnfold_minusAbsEvent20_interE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_abs->SetNameTitle("hSystUnfold_minusAbsEvent20_abs", "; Energy [MeV]; Events / 50 MeV");
   ...
   TH2D* corr_initE = new TH2D("corr_syst_minusAbsEvent_initE", "Correlations Init E; Pion initE [MeV];Pion initE [MeV];", nb_initE, eEnd, eStart, nb_initE, eEnd, eStart);
   ...
   TH2D* corr_interE = new TH2D("corr_syst_minusAbsEvent_interE", "Correlations Inter E; Pion interE [MeV];Pion interE [MeV];", nb_interE, eEnd, eStart, nb_interE, eEnd, eStart);
   ...
   TH2D* corr_abs = new TH2D("corr_syst_minusAbsEvent_abs", "Correlations Abs; Absorption interE [MeV];Absorption interE [MeV] E", nb_abs, eEnd, eStart, nb_abs, eEnd, eStart);
   ...
   legend_initE->AddEntry(hUnfold_initE, "Unfold Syst minusAbsEvent");
   ...
   legend_interE->AddEntry(hUnfold_interE, "Unfold Syst minusAbsEvent");
   ...
   legend_abs->AddEntry(hUnfold_abs, "Unfold Syst minusAbsEvent");
   ...
   TCanvas* c_initE= new TCanvas("cSyst_minusAbsEvent_initE","cSyst_minusAbsEvent_initE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_interE= new TCanvas("cSyst_minusAbsEvent_interE","cSyst_minusAbsEvent_interE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_abs= new TCanvas("cSyst_minusAbsEvent_abs","cSyst_minusAbsEvent_abs", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TH1D* hUnfold_incident= new TH1D ("hSystUnfold_minusAbsEvent_incident", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   legend_incident->AddEntry(hUnfold_incident, "Unfold Syst minusAbsEvent");
   ...
   TCanvas* c_incident= new TCanvas("cSyst_minusAbsEvent_incident","cSyst_minusAbsEvent_incident", 1100, 800);
   gPad->SetGrid(1,1);
...
#ifndef __CINT__
int main () { unfoldSystematic_bayes_minusAbsEvent(mcPath, dataPath); return 0; }  // Main program when run stand-alone
#endif
