...
void unfoldSystematic_bayes_minusAbsEfficiency(const string mcFilepath, const string dataPath, bool doMC = false, bool doXS = true)
{
   ...
   int delta_abs = (int) total_abs - new_abs;
   int cnt_abs = 0;
   ...
   frame_train
      .Foreach( [ &response_abs, &cnt_abs, &delta_abs](double true_initE, double true_interE, 
            ...
            //Signal
             //increase Miss
            if(ev_response && cnt_abs <= delta_abs){
            response_abs.Miss(true_interE);
            ++cnt_abs;
            }
            //Less events in response matrix
            else if( ev_response && cnt_abs > delta_abs ) {
   ...
   TH1D* hMeas_data_initE= new TH1D ("syst_minusAbsEfficiency_initKE", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_interE= new TH1D ("trueMC_interE", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_interE= new TH1D ("syst_minusAbsEfficiency_interKE", "Measured data Run 58XX interE; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_abs= new TH1D ("trueMC_abs", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_abs= new TH1D ("syst_minusAbsEfficiency_abs", "Measured data Run 58XX absorption; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   hUnfold_initE->SetNameTitle("hSystUnfold_minusAbsEfficiency_initE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_interE->SetNameTitle("hSystUnfold_minusAbsEfficiency_interE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_abs->SetNameTitle("hSystUnfold_minusAbsEfficiency_abs", "; Energy [MeV]; Events / 50 MeV");
   ...
   TH2D* corr_initE = new TH2D("corr_syst_minusAbsEfficiency_initE", "Correlations Init E; Pion initE [MeV];Pion initE [MeV];", nb_initE, eEnd, eStart, nb_initE, eEnd, eStart);
   ...
   TH2D* corr_interE = new TH2D("corr_syst_minusAbsEfficiency_interE", "Correlations Inter E; Pion interE [MeV];Pion interE [MeV];", nb_interE, eEnd, eStart, nb_interE, eEnd, eStart);
   ...
   TH2D* corr_abs = new TH2D("corr_syst_minusAbsEfficiency_abs", "Correlations Abs; Absorption interE [MeV];Absorption interE [MeV] E", nb_abs, eEnd, eStart, nb_abs, eEnd, eStart);
   ...
   legend_initE->AddEntry(hUnfold_initE, "Unfold Syst minusAbsEfficiency");
   ...
   legend_interE->AddEntry(hUnfold_interE, "Unfold Syst minusAbsEfficiency");
   ...
   legend_abs->AddEntry(hUnfold_abs, "Unfold Syst minusAbsEfficiency");
   ...
   TCanvas* c_initE= new TCanvas("cSyst_minusAbsEfficiency_initE","cSyst_minusAbsEfficiency_initE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_interE= new TCanvas("cSyst_minusAbsEfficiency_interE","cSyst_minusAbsEfficiency_interE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_abs= new TCanvas("cSyst_minusAbsEfficiency_abs","cSyst_minusAbsEfficiency_abs", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TH1D* hUnfold_incident= new TH1D ("hSystUnfold_minusAbsEfficiency_incident", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   legend_incident->AddEntry(hUnfold_incident, "Unfold Syst minusAbsEfficiency");
   ...
   TCanvas* c_incident= new TCanvas("cSyst_minusAbsEfficiency_incident","cSyst_minusAbsEfficiency_incident", 1100, 800);
   gPad->SetGrid(1,1);
...
#ifndef __CINT__
int main () { unfoldSystematic_bayes_minusAbsEfficiency(mcPath, dataPath); return 0; }  // Main program when run stand-alone
#endif
