...
void unfoldSystematic_bayes_plusAbsEvent(const string mcFilepath, const string dataPath, bool doMC = false, bool doXS = true)
{
   ...
   int cnt_abs_true = 0;
   frame_train
      .Foreach( [ &response_abs, &cnt_abs_true](double true_initE, double true_interE, 
               double reco_initE, double reco_interE, 
               bool ev_response, bool ev_miss, bool ev_fake){
   ...
   //===================================================================================== 
   // Increase Absorption events by 10%          
   //=====================================================================================  
   //
   //int extra_absorption = 0.1*cnt_abs_true;
   int extra_absorption = 0.2*cnt_abs_true;
   std::cout << "Adding " << extra_absorption << " Absorption Events to Response and Miss Matrix" << std::endl;
   //Filter mc sample for true_absSignal and add those to the response matrices
   
   frame_train
      .Filter("truePion_abs_response || truePion_abs_miss")
      .Range(0, 4*extra_absorption, 4) // Go from first ev to 4*nEv (40%) pick one event every 4. some randomness added like that
      .Foreach( [ &response_abs](double true_interE, 
               double reco_interE, 
            ...
            ,{ "true_interKE", "reco_interKE_rwData", 
            "truePion_abs_response", "truePion_abs_miss"});
   ...
   TH1D* hMeas_data_initE= new TH1D ("syst_plusAbsEvent_initKE", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_interE= new TH1D ("trueMC_interE", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_interE= new TH1D ("syst_plusAbsEvent_interKE", "Measured data Run 58XX interE; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_abs= new TH1D ("trueMC_abs", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_abs= new TH1D ("syst_plusAbsEvent_abs", "Measured data Run 58XX absorption; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   hUnfold_initE->SetNameTitle("hSystUnfold_plusAbsEvent20_initE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_interE->SetNameTitle("hSystUnfold_plusAbsEvent20_interE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_abs->SetNameTitle("hSystUnfold_plusAbsEvent20_abs", "; Energy [MeV]; Events / 50 MeV");
   ...
   TH2D* corr_initE = new TH2D("corr_syst_plusAbsEvent_initE", "Correlations Init E; Pion initE [MeV];Pion initE [MeV];", nb_initE, eEnd, eStart, nb_initE, eEnd, eStart);
   ...
   TH2D* corr_interE = new TH2D("corr_syst_plusAbsEvent_interE", "Correlations Inter E; Pion interE [MeV];Pion interE [MeV];", nb_interE, eEnd, eStart, nb_interE, eEnd, eStart);
   ...
   TH2D* corr_abs = new TH2D("corr_syst_plusAbsEvent_abs", "Correlations Abs; Absorption interE [MeV];Absorption interE [MeV] E", nb_abs, eEnd, eStart, nb_abs, eEnd, eStart);
   ...
   legend_initE->AddEntry(hUnfold_initE, "Unfold Syst plusAbsEvent");
   ...
   legend_interE->AddEntry(hUnfold_interE, "Unfold Syst plusAbsEvent");
   ...
   legend_abs->AddEntry(hUnfold_abs, "Unfold Syst plusAbsEvent");
   ...
   TCanvas* c_initE= new TCanvas("cSyst_plusAbsEvent_initE","cSyst_plusAbsEvent_initE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_interE= new TCanvas("cSyst_plusAbsEvent_interE","cSyst_plusAbsEvent_interE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_abs= new TCanvas("cSyst_plusAbsEvent_abs","cSyst_plusAbsEvent_abs", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TH1D* hUnfold_incident= new TH1D ("hSystUnfold_plusAbsEvent_incident", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   legend_incident->AddEntry(hUnfold_incident, "Unfold Syst plusAbsEvent");
   ...
   TCanvas* c_incident= new TCanvas("cSyst_plusAbsEvent_incident","cSyst_plusAbsEvent_incident", 1100, 800);
   gPad->SetGrid(1,1);
...
#ifndef __CINT__
int main () { unfoldSystematic_bayes_plusAbsEvent(mcPath, dataPath); return 0; }  // Main program when run stand-alone
#endif
