...
void unfoldSystematic_bayes_minusAbsPurity(const string mcFilepath, const string dataPath, bool doMC = false, bool doXS = true)
{
   ...
   //===================================================================================== 
   // Reduce Absorption Purity --> Increase Fake Selected       
   //=====================================================================================  
   //
   double total_abs = *frame_train.Filter("truePion_abs_response").Count();
   double total_bg = *frame_train.Filter("truePion_abs_fake").Count();
   double total_sel = total_abs + total_bg;
   double pur = total_abs / total_sel;
   double more_bg =  total_abs / (pur - 0.1) - total_abs;
   int extra_bg = (int) more_bg - total_bg;

   std::cout << "Purity = " << pur << std::endl;
   std::cout << "True Abs = " << total_abs << std::endl;
   std::cout << "True BG = " << total_bg << std::endl;
   std::cout << "Tot Sel = " << total_sel << std::endl;
   std::cout << "More BG = " << more_bg << std::endl;
   std::cout << "Extra BG = " << extra_bg << std::endl;
   ...
   //Add more BG
   frame_train
      .Filter("truePion_abs_fake")
      .Range(0,extra_bg)
      .Foreach( [ &response_abs](double reco_interE){
   ...
   TH1D* hMeas_data_initE= new TH1D ("syst_minusAbsPurity_initKE", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_interE= new TH1D ("trueMC_interE", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_interE= new TH1D ("syst_minusAbsPurity_interKE", "Measured data Run 58XX interE; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_abs= new TH1D ("trueMC_abs", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_abs= new TH1D ("syst_minusAbsPurity_abs", "Measured data Run 58XX absorption; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   hUnfold_initE->SetNameTitle("hSystUnfold_minusAbsPurity_initE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_interE->SetNameTitle("hSystUnfold_minusAbsPurity_interE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_abs->SetNameTitle("hSystUnfold_minusAbsPurity_abs", "; Energy [MeV]; Events / 50 MeV");
   ...
   TH2D* corr_initE = new TH2D("corr_syst_minusAbsPurity_initE", "Correlations Init E; Pion initE [MeV];Pion initE [MeV];", nb_initE, eEnd, eStart, nb_initE, eEnd, eStart);
   ...
   TH2D* corr_interE = new TH2D("corr_syst_minusAbsPurity_interE", "Correlations Inter E; Pion interE [MeV];Pion interE [MeV];", nb_interE, eEnd, eStart, nb_interE, eEnd, eStart);
   ...
   TH2D* corr_abs = new TH2D("corr_syst_minusAbsPurity_abs", "Correlations Abs; Absorption interE [MeV];Absorption interE [MeV] E", nb_abs, eEnd, eStart, nb_abs, eEnd, eStart);
   ...
   legend_initE->AddEntry(hUnfold_initE, "Unfold Syst minusAbsPurity");
   ...
   legend_interE->AddEntry(hUnfold_interE, "Unfold Syst minusAbsPurity");
   ...
   legend_abs->AddEntry(hUnfold_abs, "Unfold Syst minusAbsPurity");
   ...
   TCanvas* c_initE= new TCanvas("cSyst_minusAbsPurity_initE","cSyst_minusAbsPurity_initE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_interE= new TCanvas("cSyst_minusAbsPurity_interE","cSyst_minusAbsPurity_interE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_abs= new TCanvas("cSyst_minusAbsPurity_abs","cSyst_minusAbsPurity_abs", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TH1D* hUnfold_incident= new TH1D ("hSystUnfold_minusAbsPurity_incident", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   legend_incident->AddEntry(hUnfold_incident, "Unfold Syst minusAbsPurity");
   ...
   TCanvas* c_incident= new TCanvas("cSyst_minusAbsPurity_incident","cSyst_minusAbsPurity_incident", 1100, 800);
   gPad->SetGrid(1,1);
...
#ifndef __CINT__
int main () { unfoldSystematic_bayes_minusAbsPurity(mcPath, dataPath); return 0; }  // Main program when run stand-alone
#endif
