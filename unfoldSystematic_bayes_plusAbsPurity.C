...
void unfoldSystematic_bayes_plusAbsPurity(const string mcFilepath, const string dataPath, bool doMC = false, bool doXS = true)
{
   ...
   //===================================================================================== 
   // Increase Absorption Purity --> Reduce Fake Selected       
   //=====================================================================================  
   //
   ...
   double less_bg =  total_abs / (pur + 0.1) - total_abs;
   int cnt_bg = 0;
   ...
   std::cout << "Less BG = " << less_bg << std::endl;

   frame_train
      .Foreach( [ &response_abs, &less_bg, &cnt_bg ](double true_initE, double true_interE, 
               double reco_initE, double reco_interE, 
               bool ev_response, bool ev_miss, bool ev_fake){
            ...
            //Reduce BG
            else if( ev_fake && cnt_bg <= less_bg ){
   ...
   TH1D* hMeas_data_initE= new TH1D ("syst_plusAbsPurity_initKE", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_interE= new TH1D ("trueMC_interE", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_interE= new TH1D ("syst_plusAbsPurity_interKE", "Measured data Run 58XX interE; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);

   TH1D* hTrue_abs= new TH1D ("trueMC_abs", "MC True; Energy [MeV]; Events / 50 MeV",    nBin_int, eEnd, eStart);
   TH1D* hMeas_data_abs= new TH1D ("syst_plusAbsPurity_abs", "Measured data Run 58XX absorption; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   hUnfold_initE->SetNameTitle("hSystUnfold_plusAbsPurity_initE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_interE->SetNameTitle("hSystUnfold_plusAbsPurity_interE", "; Energy [MeV]; Events / 50 MeV");
   hUnfold_abs->SetNameTitle("hSystUnfold_plusAbsPurity_abs", "; Energy [MeV]; Events / 50 MeV");
   ...
   TH2D* corr_initE = new TH2D("corr_syst_plusAbsPurity_initE", "Correlations Init E; Pion initE [MeV];Pion initE [MeV];", nb_initE, eEnd, eStart, nb_initE, eEnd, eStart);
   ...
   TH2D* corr_interE = new TH2D("corr_syst_plusAbsPurity_interE", "Correlations Inter E; Pion interE [MeV];Pion interE [MeV];", nb_interE, eEnd, eStart, nb_interE, eEnd, eStart);
   ...
   TH2D* corr_abs = new TH2D("corr_syst_plusAbsPurity_abs", "Correlations Abs; Absorption interE [MeV];Absorption interE [MeV] E", nb_abs, eEnd, eStart, nb_abs, eEnd, eStart);
   ...
   legend_initE->AddEntry(hUnfold_initE, "Unfold Syst plusAbsPurity");
   ...
   legend_interE->AddEntry(hUnfold_interE, "Unfold Syst plusAbsPurity");
   ...
   legend_abs->AddEntry(hUnfold_abs, "Unfold Syst plusAbsPurity");
   ...
   TCanvas* c_initE= new TCanvas("cSyst_plusAbsPurity_initE","cSyst_plusAbsPurity_initE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_interE= new TCanvas("cSyst_plusAbsPurity_interE","cSyst_plusAbsPurity_interE", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TCanvas* c_abs= new TCanvas("cSyst_plusAbsPurity_abs","cSyst_plusAbsPurity_abs", 1100, 800);
   gPad->SetGrid(1,1);
   ...
   TH1D* hUnfold_incident= new TH1D ("hSystUnfold_plusAbsPurity_incident", "; Energy [MeV]; Events / 50 MeV", nBin_int, eEnd, eStart);
   ...
   legend_incident->AddEntry(hUnfold_incident, "Unfold Syst plusAbsPurity");
   ...
   TCanvas* c_incident= new TCanvas("cSyst_plusAbsPurity_incident","cSyst_plusAbsPurity_incident", 1100, 800);
   gPad->SetGrid(1,1);
...
#ifndef __CINT__
int main () { unfoldSystematic_bayes_plusAbsPurity(mcPath, dataPath); return 0; }  // Main program when run stand-alone
#endif
